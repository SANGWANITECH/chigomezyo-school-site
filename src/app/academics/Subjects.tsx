...
export default function SubjectsSection() {
  ...
  return (
    <section className="bg-gray-20 py-10 md:py-16 px-4">
      <div className="max-w-6xl mx-auto">

        {/* Heading */}
        <header className="mb-8 md:mb-10">
          <h2 className="text-3xl md:text-5xl font-bold text-gray-900 font-poppins leading-tight">
            <span className="inline-block bg-blue-300/30 text-blue-900/90 px-4 py-1 rounded-lg">
              Subjects
            </span>{' '}
            We Teach At
          </h2>
          <h3 className="text-3xl md:text-5xl font-bold text-gray-900 mt-1">
...
<div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
  ...
  {subjects.slice(0, showAll ? subjects.length : 6).map((subject) => (
    <div
      key={subject}
      className="bg-white border border-blue-900/70 rounded-xl p-4 flex flex-col items-start gap-3 shadow-sm md:hidden"
    >
      <BookOpen className="text-blue-900/70" size={28} />
  ...
  {subjects.map((subject) => (
    ...
    >
      <BookOpen className="text-blue-900/70" size={30} />
